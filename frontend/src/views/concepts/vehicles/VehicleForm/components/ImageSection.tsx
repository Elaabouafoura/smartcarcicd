...
import { apiUploadVehicleImage } from '@/services/DashboardService'
...
const ImageSection = ({ control, errors }: ImageSectionProps) => {
        ...
        if (file) {
            for (const f of file) {
                if (!allowedFileType.includes(f.type)) {
                    valid = 'Please upload a .jpeg, .jpg, .png or .webp file!'
                }

                if (f.size >= maxFileSize) {
                    valid = 'Upload image cannot be more than 2MB!'
                }
            }
        }
    ...
    const handleUpload = async (
        ...
        try {
            ...
            if (originalImageList.length > 0) {
                const prevImgId =
                    originalImageList[originalImageList.length - 1].id
            ...
            const uploaded = await apiUploadVehicleImage(files[latestUpload])

            const image = {
                id: imageId,
                name: files[latestUpload].name,
                img: uploaded.photoUrl,
            }
    ...
    return (
        <Card>
            <h4 className="mb-2">Vehicle Image</h4>
            <p>Choose a vehicle photo or simply drag and drop it here.</p>
            ...
            <p>
                Image formats: .jpg, .jpeg, .png, .webp. Maximum size 2MB.
            </p>
        </Card>
    )
}
